...
<body>
  ...
  <div class="modal-dialog" role="document">
	...
	<div class="row my-3 mx-1">
		<div class="col-4">
			<ul class="list-group">
				<li class="list-group-item active">Договоры</li>
...
<form>
	...
	<div class="form-group row">
		<label for="contractDate" class="col-sm-2 col-form-label">Дата заключения договора</label>
		<div class="col-sm-10">
			<input  type="text"  readonly  class="form-control-plaintext" id="contractDate" value="<?= $contract->getAgreed() ?>">
		</div>
	</div>
	<div class="form-group row">
		<label  for="supplier"  class="col-sm-2  col-formlabel">Поставщик</label>
		<div class="col-sm-10">
			<input  type="text"  readonly  class="form-control-plaintext" id="supplier" value="<?= htmlspecialchars($supplier->getName()) ?>">
		</div>
	</div>
	<div class="form-group row">
		<label for="title" class="col-sm-2 col-form-label">Название договора</label>
		<div class="col-sm-10">
			<input  type="text"  readonly  class="form-control-plaintext" id="title" value="<?= htmlspecialchars($contract->getTitle()) ?>">
		</div>
	</div>
	<div class="form-group row">
		<label for="note" class="col-sm-2 col-form-label">Заметки</label>
		<div class="col-sm-10">
			<textarea class="form-control" readonly rows="5" id="note"><?= htmlspecialchars($contract->getNote()) ?></textarea>
		</div>
	</div>
</form> 
<a class="btn btn-warning <?php if(strcmp($_SESSION['username'],'market_manager')==0) echo 'disabled' ?>" href="contracts.php?edit=<?= $contract->getNumber() ?>"  role="button">Редактировать</a>
<a class="btn btn-danger <?php if(strcmp($_SESSION['username'],'market_manager')==0) echo 'disabled' ?>" href="contracts.php?delete=<?= $contract->getNumber() ?>" role="button" >Удалить</a>
<?php
}
catch (Exception $e)
{
...
if (isset($_GET['new']))
	...
	<button type="submit" class="btn btn-primary" name="saveC">Сохранить</button>
...
<?php 	} else{
	...
	<button type="submit" class="btn btn-primary" name="updateC">Сохранить</button>
...
?><a class="btn btn-success <?php if(strcmp($_SESSION['username'],'market_manager')==0) echo 'disabled' ?>" href="contracts.php?new" role="button">Новый договор</a><?php
 	}
 }
}
if (isset($_GET['delete']))
 {
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>Вы действительно хотите удалить информацию о договоре №<?=$contract->getNumber()?>?</p>
      </div>
      <div class="modal-footer">
      	<form action="contracts.php" method="post">
      		<input type="text" hidden name="number" value="<?=$contract->getNumber()?>">
        <button type="submit" class="btn btn-primary" name="deleteC">Удалить</button>
    </form>
